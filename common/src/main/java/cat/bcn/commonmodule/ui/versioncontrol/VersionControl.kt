    ...
    actual fun check(
        ...
        GlobalScope.launch {
            val version: Version = buildClient(VERSION_CONTROL_ENDPOINT).use {
                it.get("$VERSION_ROUTE/$appId/$platform/$versionCode")
            }
            ...
            when (version.comparisonMode) {
                Version.ComparisonMode.FORCE -> dialog.setCancelable(true)
                Version.ComparisonMode.LAZY -> dialog.setNegativeButton(version.cancel(language)) { _, _ -> f(CANCELLED) }
